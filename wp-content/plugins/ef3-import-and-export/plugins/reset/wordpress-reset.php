...
Plugin URI: http://sivel.net/wordpress/ef3-wordpress-reset/
...
class EF3_WordPress_Reset {
	...
	public function favorites( $actions ) {
		$reset['tools.php?page=ef3-wordpress-reset'] = array( esc_html__( 'WordPress Reset' ), 'level_10' );
	...
	public function admin_bar_link() {
		...
		$wp_admin_bar->add_menu(
			array(
				'parent' => 'site-name',
				'id'     => 'ef3-wordpress-reset',
				'title'  => 'Reset Site',
				'href'   => admin_url( 'tools.php?page=ef3-wordpress-reset' )
			)
		);
	}
	...
	public function admin_init() {
		...
		if ( array_key_exists( 'reset', $_GET ) && stristr( $_SERVER['HTTP_REFERER'], 'ef3-wordpress-reset' ) ) {
	...
	public function add_page() {
		if ( current_user_can( 'level_10' ) && function_exists( 'add_management_page' ) )
			$hook = add_management_page( 'Reset', 'Reset', 'level_10', 'ef3-wordpress-reset', array( &$this, 'admin_page' ) );
...
$WordPress_Reset = new EF3_WordPress_Reset();
